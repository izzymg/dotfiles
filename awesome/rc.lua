...
local naughty = require("naughty") -- notifs
...
-- Clock
local clock_widget = wibox.widget.textclock("<span foreground='#fff'> %H:%M</span> | %d/%m %Y")

-- CPU
local cpu_widget = awful.widget.graph()
cpu_widget:set_width(50)
cpu_widget:set_background_color(beautiful.bg_normal)
cpu_widget:set_color{ type = "linear", from = { 0, 0 }, to = { 50, 0 },
                     stops = { { 0, beautiful.cpu_low },
                               { 0.5, beautiful.cpu_mid },
                               { 1, beautiful.cpu_high } } }
...
-- Update task list widgets 
function task_update(w, buttons, label, data, objects) 
	common.list_update(w, buttons, label, data, objects)
	prints("bingus")
	w:set_max_widget_size(900)
	w:set_min_widget_size(900)
end
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create the wibox
    s.top_bar = awful.wibar({ position = "top", screen = s, height = 35 })
...
client.connect_signal("property::name", function(c)
	c.name = string.lower(c.class or c.name)
end)
...
globalkeys = gears.table.join(
	...
	-- audio
	awful.key({ modkey }, "Up", function () volume_widget:inc(1) end),
	awful.key({ modkey }, "Down", function () volume_widget:dec(1) end),
...
awful.rules.rules = {
    ...
    -- Only want titlebars on dialogs
